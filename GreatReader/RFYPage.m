...
- (RFYPageLink *)linkAtPoint:(CGPoint)point
{
    ...
    __block RFYPageLink *hit = nil;
    [self.linkList.links enumerateObjectsUsingBlock:^(RFYPageLink *link, NSUInteger idx, BOOL *stop) {
...
- (void)updateLinkFrames
{
    ...
    NSArray *linkFrames = [self.linkList.links grt_map:^(RFYPageLink *link) {
...
- (void)updateCharacterFrames
{
    ...
    dispatch_async(queue, ^{
        NSArray *frames = [characters grt_map:^(RFYRenderingCharacter *character) {
...
{
    __block RFYRenderingCharacter *c = nil;
...
- (void)selectWordForCharacter:(RFYRenderingCharacter *)character
{
    ...
    while (from > 0) {
        RFYRenderingCharacter *c = [self.characters objectAtIndex:from - 1];
    ...
    while (to < self.characters.count - 1) {
        RFYRenderingCharacter *c = [self.characters objectAtIndex:to + 1];
        if (c.isNotWordCharater || ![c isSameLineAs:character]) {
            break;
        }
        to++;
...
- (NSString *)selectedString
{
    ...
    for (RFYRenderingCharacter *c in self.selectedCharacters) {
