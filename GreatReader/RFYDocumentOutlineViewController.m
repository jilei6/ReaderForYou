//
//  RFYDocumentOutlineViewController.m
//  GreatReader
//
...
@interface RFYDocumentOutlineViewController ()
@property (nonatomic, strong) NSArray *outlineItems;
@property (nonatomic, assign) NSInteger activeIndex;
@property (nonatomic, strong) IBOutlet UISegmentedControl *segmentedControl;
@end

@implementation RFYDocumentOutlineViewController

- (void)viewDidLoad
{
    [super viewDidLoad];

    NSString *nibName = @"RFYDocumentOutlineItemCell";
...
{
    __block __weak NSArray * (^weak_subtrees)(RFYDocumentOutlineItem *, NSUInteger);
    NSArray * (^subtrees)(RFYDocumentOutlineItem *, NSUInteger) =
            ^(RFYDocumentOutlineItem *item, NSUInteger level) {
        NSMutableArray *array = [NSMutableArray arrayWithObject:@{
            @"item": item, @"level": @(level)
        }];
        for (RFYDocumentOutlineItem *c in item.children) {
    ...
    for (RFYDocumentOutlineItem *item in self.outline.items) {
...
- (void)setOutline:(RFYDocumentOutline *)outline
{
    _outline = outline;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RFYDocumentOutlineItem *item = dic[@"item"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:PDFDocumentOutlineSegueExit]) {
        if (sender == self) {
            ...
            RFYDocumentViewController *vc = segue.destinationViewController;
