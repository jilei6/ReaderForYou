...
@property (nonatomic, strong) RFYCMap *cmap;
@property (nonatomic, assign) NSStringEncoding encoding;
@property (nonatomic, strong) NSString *baseFont;
@end

@implementation RFYFont

+ (RFYFont *)fontWithFontDictionary:(CGPDFDictionaryRef)fontDictionary
{
...
- (RFYFontDescriptor *)fontDescriptorWithFontDictionary:(CGPDFDictionaryRef)fontDictionary
{
    CGPDFDictionaryRef dic = PDFDictionaryGetDictionary(fontDictionary, "FontDescriptor");
    RFYFontDescriptor *fontDescriptor =
            [[RFYFontDescriptor alloc] initWithFontDescriptorDictionary:dic];
    return fontDescriptor;
}

- (RFYCMap *)cmapWithFontDictionary:(CGPDFDictionaryRef)fontDictionary
{
    CGPDFStreamRef stream = PDFDictionaryGetStream(fontDictionary, "ToUnicode");
    if (stream) {
        RFYCMap *cmap = [[RFYCMap alloc] initWithStream:stream];
        return cmap;
    }
    return nil;
}
...
- (RFYFontDescriptor *)fontDescriptorWithFontDictionary:(CGPDFDictionaryRef)fontDictionary
{
    CGPDFDictionaryRef dic = PDFDictionaryGetDictionary(fontDictionary, "FontDescriptor");
    if (dic) {
        return [[RFYFontDescriptor alloc] initWithFontDescriptorDictionary:dic];
    } else {
        NSString *baseFont = [self baseFontWithFontDictionary:fontDictionary];
        return [[RFYFontDescriptor alloc] initWithBaseFont:baseFont];
    }
}
...
@property (nonatomic, readonly) RFYFont *descendantFont;
@end
...
- (NSArray *)descendantFontsWithFontDictionary:(CGPDFDictionaryRef)fontDictionary
{
    ...
    if (descendantFont) {
        return @[
            [RFYFont fontWithFontDictionary:descendantFont]
        ];
    } else {
        return nil;
    }
