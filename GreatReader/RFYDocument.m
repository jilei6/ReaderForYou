...
@interface RFYDocument ()
@property (nonatomic, strong) RFYDocumentNameList *nameList;
@property (nonatomic, assign, readwrite) NSUInteger numberOfPages;
@property (nonatomic, strong, readwrite) UIImage *thumbnailImage;
@property (nonatomic, strong, readwrite) RFYDocumentOutline *outline;
@property (nonatomic, strong, readwrite) RFYDocumentCrop *crop;
@property (nonatomic, strong, readwrite) RFYDocumentBookmarkList *bookmarkList;
@property (nonatomic, assign, readwrite) CGPDFDocumentRef CGPDFDocument;
@property (nonatomic, strong, readwrite) RFYDocumentSearch *search;
@property (nonatomic, copy, readwrite) NSString *title;
@property (nonatomic, assign, readwrite) NSUInteger currentPage;
@property (nonatomic, strong, readwrite) RFYDocumentBackForwardList *backForwardList;
@property (nonatomic, strong) FBKVOController *kvoController;
@end

@implementation RFYDocument
...
{
    NSString *path = [RFYDocument absolutePathWithRelativePath:
...
{
    NSString *path = [RFYDocument relativePathWithAbsolutePath:self.path];
    NSAssert(path, @"");
...
- (RFYPage *)pageAtIndex:(NSUInteger)index
{
    ...
    if (cgPage) {
        RFYPage *page = [[RFYPage alloc] initWithCGPDFPage:cgPage];
        page.linkList = [[RFYPageLinkList alloc] initWithCGPDFPage:cgPage
...
- (RFYDocumentOutline *)outline
{
    if (!_outline) {
        _outline = [[RFYDocumentOutline alloc]
                       initWithCGPDFDocument:self.CGPDFDocument];
    }
    return _outline;
}

- (RFYDocumentCrop *)crop
{
    if (!_crop) {
        _crop = [[RFYDocumentCrop alloc] init];
    }
    return _crop;
}

- (RFYDocumentBookmarkList *)bookmarkList
{
    if (!_bookmarkList) {
        _bookmarkList = [RFYDocumentBookmarkList new];
        _bookmarkList.document = self;
    }
    return _bookmarkList;
}

- (RFYDocumentSearch *)search
{
    if (!_search) {
        _search = [[RFYDocumentSearch alloc] initWithDocument:self];
    }
    return _search;
}

- (RFYDocumentBackForwardList *)backForwardList
{
    if (!_backForwardList) {
        _backForwardList = [[RFYDocumentBackForwardList alloc] initWithCurrentPage:self.currentPage];
        [self.kvoController observe:_backForwardList
                            keyPath:@"currentPage"
                            options:0
                              block:^(RFYDocument *document, RFYDocumentBackForwardList *list, NSDictionary *change) {
...
- (RFYDocumentNameList *)nameList
{
    if (!_nameList) {
        _nameList = [[RFYDocumentNameList alloc] initWithCGPDFDocument:self.CGPDFDocument];        
    }
    return _nameList;
}
...
{
    RFYPage *page = [self pageAtIndex:1];
...
{
    if ([anObject isKindOfClass:[RFYDocument class]]) {
        RFYDocument *doc = (RFYDocument *)anObject;
