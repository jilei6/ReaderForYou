...
#import "RFYDocumentBookmarkList.h"
#import "RFYDocumentBookmarkListViewController.h"
#import "RFYDocumentCrop.h"
#import "RFYDocumentCropViewController.h"
#import "RFYDocumentInfo.h"
#import "RFYDocumentInfoView.h"
#import "RFYDocumentBrightnessViewController.h"
...
#import "RFYDocumentSearchViewController.h"
...
@interface RFYDocumentViewController () <UIPageViewControllerDataSource,
...
@property (nonatomic, strong) IBOutlet RFYDocumentPageSlider *slider;
@property (nonatomic, strong) IBOutlet UIView *dimView;
@property (nonatomic, strong) RFYDocumentInfoView *infoView;
@end

@implementation RFYDocumentViewController
...
willTransitionToViewControllers:(NSArray *)pendingViewControllers
{
    ...
    for (RFYPageViewController *vc in pendingViewControllers) {
...
{
    RFYPageViewController *vc = pageViewController.viewControllers.firstObject;
    [self.document goTo:vc.page.index addHistory:YES];
}

#pragma mark -

- (RFYPageViewController *)currentPageViewController
{
    RFYPageViewController *vc = self.pageViewController.viewControllers.firstObject;
    return vc;
}

#pragma mark - Init RFYPageViewController

- (RFYPageViewController *)pageViewControllerAtIndex:(NSUInteger)index
{
    RFYPage *page = [self.document pageAtIndex:index];    
    if (!page) {
        return nil;
    }
    page.crop = self.document.crop;    
    RFYPageViewController *vc =
            [[RFYPageViewController alloc] initWithPage:page];
...
- (void)prepareToolbar
{
    ...
    self.slider.model = [[RFYDocumentPageSliderModel alloc] initWithDocument:self.document];
}
...
{
    RFYDocumentInfo *info = [[RFYDocumentInfo alloc] initWithDocument:self.document];    
    if (!self.infoView) {
        self.infoView = [[RFYDocumentInfoView alloc] initWithFrame:self.view.bounds];
...
{   
    if ([segue.identifier isEqualToString:PDFDocumentViewControllerSegueOutline]) {
        ...
        RFYDocumentOutlineContainerViewController *vc =
                (RFYDocumentOutlineContainerViewController *)navi.topViewController;
        vc.outlineViewController.currentPage = self.document.currentPage;
        vc.outlineViewController.outline = [[RFYDocumentOutline alloc] initWithCGPDFDocument:self.document.CGPDFDocument];
        ...
        UINavigationController *navi =
                (UINavigationController *)segue.destinationViewController;
        RFYDocumentCropViewController *vc =
                (RFYDocumentCropViewController *)navi.topViewController;
        ...
        RFYDocumentBrightnessViewController *vc =
                (RFYDocumentBrightnessViewController *)segue.destinationViewController;
        vc.document = self.document;
        ...
        UINavigationController *navi =
                (UINavigationController *)segue.destinationViewController;
        RFYDocumentSearchViewController *vc =
                (RFYDocumentSearchViewController *)navi.topViewController;
        vc.viewModel = [[RFYDocumentSearchViewModel alloc] initWithSearch:self.document.search
...
#pragma mark - RFYDocumentPageSlider Delegate, DataSouce
