...
- (void)openDocumentsAtURL:(NSURL *)URL
{
    for (int i = 0; i < self.viewModel.count; i++) {
        RFYDocument *document = [self.viewModel documentAtIndex:i];
...
- (id<DocumentCell>)documentCellForDocument:(RFYDocument *)document
{
    for (DocumentCollectionViewCell *cell in [self.collectionView visibleCells]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RFYDocument *document = [self.viewModel documentAtIndex:indexPath.item];
